...
function mockFetch(routes: Record<string, { status?: number; body: unknown; headers?: Record<string, string> } | (() => { status?: number; body: unknown })>) {
  ...
  const impl = (async (input: string | URL | Request, init?: RequestInit) => {
    ...
    return new Response(JSON.stringify(r.body), { status: r.status ?? 200, headers: { 'content-type': 'application/json', ...(('headers' in r ? r.headers : undefined) as Record<string, string> | undefined) } });
  }) as typeof fetch;
  return { impl, calls };
}

const noWaitLimiter = () => new RateLimiter(60, () => 0, async () => {});
...
describe('discogs mapper', () => {
  it('maps a release into the domain shape', () => {
    ...
    expect(r.images[0]).toMatchObject({ kind: 'primary', url: 'https://i.discogs.com/primary.jpg' });
  });

  it('parses search results and durations defensively', () => {
    expect(
      mapSearchResult({ id: 1, type: 'release', title: 'Love - Forever Changes', year: '1967', label: ['Elektra', 'Elektra'], catno: 'EKS-74013', format: ['Vinyl', 'LP'], barcode: ['07559 74013 2'] }),
    ).toMatchObject({ artist: 'Love', title: 'Forever Changes', year: 1967, labels: ['Elektra'], barcodes: ['07559740132'] });
  ...
  it('maps price suggestions to Goldmine grades', () => {
    expect(
      mapPriceSuggestions({ 'Near Mint (NM or M-)': { currency: 'USD', value: 301.456 }, 'Very Good Plus (VG+)': { currency: 'USD', value: 200 }, Weird: { currency: 'USD', value: 1 } }),
    ).toEqual([
...
describe('DiscogsService', () => {
  it('sends auth + user agent and maps search params', async () => {
    const f = mockFetch({ '/database/search': { body: { pagination: { page: 1, pages: 1, items: 1 }, results: [{ id: 5, type: 'release', title: 'A - B' }, { id: 6, type: 'artist', title: 'A' }] } } });
    const svc = new DiscogsService({ token: 'tok', userAgent: 'Kollektor/test', fetch: f.impl, limiter: noWaitLimiter() });
    const res = await svc.search({ type: 'release', catalogNumber: 'SHVL 804', barcode: '5 0999-02', page: 1, perPage: 20 });
  ...
  it('retries on 429 and succeeds', async () => {
    let n = 0;
    const f = mockFetch({ '/releases/1873013': () => (n++ === 0 ? { status: 429, body: {} } : { body: release }) });
    const svc = new DiscogsService({ token: 't', userAgent: 'u', fetch: f.impl, limiter: noWaitLimiter() });
  ...
  it('falls back from price suggestions to lowest listing when not allowed', async () => {
    const f = mockFetch({
      '/marketplace/price_suggestions/1': { status: 403, body: { message: 'seller settings required' } },
      '/marketplace/stats/1': { body: { lowest_price: { currency: 'USD', value: 20 }, num_for_sale: 3 } },
    });
    const svc = new DiscogsService({ token: 't', userAgent: 'u', fetch: f.impl, limiter: noWaitLimiter() });
    expect(await svc.getMarketValues('1')).toEqual([{ kind: 'lowest', condition: null, amount: 20, currency: 'USD' }]);
  });
});
...
describe('RateLimiter', () => {
  it('spaces requests and pauses when remaining quota is low', async () => {
    let t = 0;
    const waits: number[] = [];
    const rl = new RateLimiter(60, () => t, async (ms) => { waits.push(ms); });
    await rl.acquire();
    await rl.acquire();
    await rl.acquire();
