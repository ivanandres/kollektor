...
const add = (input: Record<string, unknown>) =>
  ctx.core.wishlist.add(ctx.userId, addToWishlistInput.parse(input));

describe('wishlist', () => {
  it('adds any edition of an album (Discogs master) or a specific edition', async () => {
    const any = await add({
      discogsMasterId: 10414,
      priority: 1,
      targetPrice: 40,
      targetCurrency: 'USD',
    });
  ...
  it('moves through statuses and converts to a collection item on purchase', async () => {
    ...
    await expect(
      ctx.core.wishlist.update(ctx.userId, w.id, { status: 'purchased' }),
  ...
  it('refuses to purchase an edition of a different album', async () => {
    ...
    await expect(
      ctx.core.wishlist.purchase(ctx.userId, w.id, { discogsReleaseId: 1873013 }),
    ).rejects.toMatchObject({ code: 'VALIDATION' });
  });
});
