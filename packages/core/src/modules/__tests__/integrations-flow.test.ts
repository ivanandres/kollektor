...
describe('music links', () => {
  it('returns only verified matches and caches both hits and misses', async () => {
    ...
    seedLibrary(ctx.catalog);
    const { item } = await ctx.core.collection.add(ctx.userId, addToCollectionInput.parse({ discogsReleaseId: 1873013 }));
    ...
    expect(r.track).toMatchObject({ artist: 'Pink Floyd', title: 'Money', album: 'The Dark Side Of The Moon', durationSeconds: 382 });
    expect(r.links.spotify).toEqual({ status: 'found', url: 'https://open.spotify.com/track/abc', confidence: 0.95 });
    ...
    expect(low.links.spotify).toEqual({ status: 'not_found', message: 'No encontramos este tema.' });
...
describe('recognition', () => {
  it('photo → hints → candidate editions to confirm (never auto-selected)', async () => {
    const recognizer = new FakeRecognizer({ artist: 'Pink Floyd', title: 'Dark Side of the Moon', catalogNumber: 'SHVL 804' });
    await h.setup({ recognizer, config: { visionDailyLimit: 2 } });
    seedLibrary(ctx.catalog);
    const r = await ctx.core.recognition.identifyByPhoto(ctx.userId, [{ data: 'AAAA', mediaType: 'image/jpeg' }]);
    ...
    await ctx.core.recognition.identifyByPhoto(ctx.userId, [{ data: 'AAAA', mediaType: 'image/jpeg' }]);
    await expect(ctx.core.recognition.identifyByPhoto(ctx.userId, [{ data: 'AAAA', mediaType: 'image/jpeg' }])).rejects.toMatchObject({ code: 'RATE_LIMITED' });
    // Quota is per user
    const other = await createUser(h.db);
    await expect(ctx.core.recognition.identifyByPhoto(other, [{ data: 'AAAA', mediaType: 'image/jpeg' }])).resolves.toBeTruthy();
  });

  it('barcode lookup needs no vision call', async () => {
    ctx.catalog.addRelease({ id: '999', artist: 'Radiohead', title: 'OK Computer', year: 1997, barcode: '724385522925' });
  ...
  it('fails clearly when vision is not configured', async () => {
    await expect(ctx.core.recognition.identifyByPhoto(ctx.userId, [])).rejects.toMatchObject({ code: 'NOT_CONFIGURED' });
  });
});
...
describe('profiles', () => {
  it('creates a private profile with a unique username', async () => {
    ...
    expect(p).toMatchObject({ profileVisibility: 'private', collectionVisibility: 'private', showPrices: false, baseCurrency: 'USD' });
    ...
    await expect(ctx.core.profiles.updateProfile(other, { username: p.username })).rejects.toMatchObject({ code: 'CONFLICT' });
  });

  it('public profile hides everything until the owner opts in', async () => {
    const p = await ctx.core.profiles.getProfile(ctx.userId);
    await expect(ctx.core.profiles.getPublicProfile(p.username)).rejects.toMatchObject({ code: 'NOT_FOUND' });
    await ctx.core.profiles.updateProfile(ctx.userId, { profileVisibility: 'public', bio: 'Coleccionista de prog' });
    const pub = await ctx.core.profiles.getPublicProfile(p.username);
    expect(pub).toEqual({ username: p.username, displayName: 'Test User', avatarUrl: null, bio: 'Coleccionista de prog', collectionVisible: false, wishlistVisible: false });
  });
});
