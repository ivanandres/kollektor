...
import { addToCollectionInput, collectionQuery, updateCollectionItemInput } from '@kollektor/schemas';
...
export function collectionRoutes({ core }: AppDeps) {
  ...
  return new Hono<AppEnv>()
    .get('/', async (c) => c.json(await core.collection.list(c.get('userId'), parse(collectionQuery, queryObject(c)))))
    ...
    .get('/:id{[0-9a-f-]{36}}', async (c) => c.json(await core.collection.get(c.get('userId'), c.req.param('id'))))
    .patch('/:id{[0-9a-f-]{36}}', async (c) => {
      const input = parse(updateCollectionItemInput, await jsonBody(c));
      return c.json(await withAchievements(await core.collection.update(c.get('userId'), c.req.param('id'), input)));
    })
