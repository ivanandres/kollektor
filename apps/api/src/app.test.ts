...
import { FakeCatalog, FakeFx, FakeMusicLinks, FakeRecognizer, seedLibrary } from '@kollektor/core/testing';
...
const core = createCore({
  ...
  marketValue: catalog,
  musicLinks: [new FakeMusicLinks('spotify', { Money: { url: 'https://open.spotify.com/track/money', externalId: 'money', confidence: 0.9 } })],
...
async function call(path: string, init: { method?: string; body?: unknown; session?: Session; bearer?: boolean; headers?: Record<string, string> } = {}) {
  ...
  const res = await app.request(`/api${path}`, { method: init.method ?? 'GET', headers, body: init.body !== undefined ? JSON.stringify(init.body) : undefined });
  const text = await res.text();
  return { status: res.status, headers: res.headers, json: text ? JSON.parse(text) : null };
}

async function signUp(name: string, emailAddr: string, password = 'vinilos-1973'): Promise<Session> {
  const r = await call('/auth/sign-up/email', { method: 'POST', body: { name, email: emailAddr, password } });
...
describe('auth & profile', () => {
  ...
  it('creates a private profile on sign-up and lets the user edit it', async () => {
    ...
    expect(p.json).toMatchObject({ username: 'ivan_andres', profileVisibility: 'private', collectionVisibility: 'private', showPrices: false });
    const upd = await call('/me/profile', { method: 'PATCH', session: ivan, body: { username: 'ivan', bio: 'Prog y jazz', baseCurrency: 'usd' } });
    expect(upd.json).toMatchObject({ username: 'ivan', bio: 'Prog y jazz', baseCurrency: 'USD' });
    expect((await call('/me/profile', { method: 'PATCH', session: ivan, body: { username: 'no' } })).status).toBe(400);
  });
  ...
  it('password reset: email with link → new password works', async () => {
    ...
    void other;
    const r = await call('/auth/request-password-reset', { method: 'POST', body: { email: 'forgot@example.com', redirectTo: `${ORIGIN}/reset` } });
    expect(r.status).toBe(200);
    const mail = email.outbox.at(-1)!;
    expect(mail.to).toBe('forgot@example.com');
    const token = /reset-password\/([^?\s]+)/.exec(mail.text!)![1]!;
    const reset = await call('/auth/reset-password', { method: 'POST', body: { token, newPassword: 'new-password-2' } });
    expect(reset.status).toBe(200);
    expect((await call('/auth/sign-in/email', { method: 'POST', body: { email: 'forgot@example.com', password: 'old-password-1' } })).status).toBe(401);
    expect((await call('/auth/sign-in/email', { method: 'POST', body: { email: 'forgot@example.com', password: 'new-password-2' } })).status).toBe(200);
  });
});
...
describe('MVP flow', () => {
  ...
  it('finds an edition (search / photo / barcode) and adds it with the price paid', async () => {
    const search = await call('/catalog/external/search?artist=pink%20floyd&title=dark%20side', { session: ivan });
    expect(search.status).toBe(200);
    expect(search.json.attribution).toBe('Datos provistos por Discogs');
    expect(search.json.items.map((i: { country: string }) => i.country).sort()).toEqual(['Japan', 'UK']);

    const photo = await call('/catalog/identify/photo', { method: 'POST', session: ivan, body: { images: [{ data: 'data:image/jpeg;base64,AAAA', mediaType: 'image/jpeg' }] } });
    ...
    const added = await call('/collection', {
      method: 'POST',
      session: ivan,
      body: { discogsReleaseId: 1873013, purchasePrice: 35, purchaseCurrency: 'USD', purchaseDate: '2026-09-20', conditionMedia: 'VG+', purchasePlace: 'Disquería del centro' },
    });
    expect(added.status).toBe(201);
    expect(added.json.unlockedAchievements.map((a: { code: string }) => a.code)).toContain('count-1');
  ...
  it('shows the full record with tracklist and verified listening links', async () => {
    ...
    expect(item.json.release.album).toMatchObject({ title: 'The Dark Side Of The Moon', artistDisplay: 'Pink Floyd' });
    expect(item.json.release.labels[0]).toMatchObject({ name: 'Harvest', catalogNumber: 'SHVL 804' });
    ...
    expect(links.json.links.spotify).toMatchObject({ status: 'found', url: 'https://open.spotify.com/track/money' });
    const breathe = item.json.release.tracks.find((t: { title: string }) => t.title === 'Breathe');
    expect((await call(`/catalog/tracks/${breathe.id}/links`, { session: ivan })).json.links.spotify).toEqual({ status: 'not_found', message: 'No encontramos este tema.' });
  });

  it('searches and filters the collection', async () => {
    await call('/collection', { method: 'POST', session: ivan, body: { discogsReleaseId: 2000006, purchasePrice: 40000, purchaseCurrency: 'ARS' } });
    await call('/collection', { method: 'POST', session: ivan, body: { discogsReleaseId: 2000007 } });
    ...
    const multi = await call('/collection?country=UK&country=Japan&sort=year_asc', { session: ivan });
    ...
    expect(facets.json.countries.map((x: { value: string }) => x.value).sort()).toEqual(['Japan', 'UK']);
  });

  it('wishlist → purchase → collection', async () => {
    const w = await call('/wishlist', { method: 'POST', session: ivan, body: { discogsMasterId: 10414, priority: 1, targetPrice: 45, targetCurrency: 'USD' } });
    expect(w.status).toBe(201);
    const bought = await call(`/wishlist/${w.json.id}/purchase`, { method: 'POST', session: ivan, body: { discogsReleaseId: 2000002, purchasePrice: 42, purchaseCurrency: 'USD' } });
    expect(bought.status).toBe(201);
    expect((await call('/wishlist', { session: ivan })).json).toHaveLength(0);
    expect((await call('/wishlist?includePurchased=true', { session: ivan })).json[0].status).toBe('purchased');
  });
  ...
  it('dashboard, achievements and discovery', async () => {
    ...
    expect(disc.json.some((i: { message: string }) => i.message === 'Tenés 2 de los 8 discos esenciales de Pink Floyd.')).toBe(true);
  });
  ...
  it('isolates users and never leaks one collection to another', async () => {
    ...
    expect((await call(`/collection/${itemId}`, { method: 'DELETE', session: eve })).status).toBe(404);
  ...
  it('protects the cron endpoint', async () => {
    ...
    const ok = await call('/cron/maintenance', { headers: { authorization: 'Bearer cron-secret' } });
    expect(ok.status).toBe(200);
  });

  it('validation errors are structured', async () => {
    const r = await call('/collection', { method: 'POST', session: ivan, body: { purchasePrice: 10 } });
